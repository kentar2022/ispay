// Функции для отображения данных курса
function displayCourseData(data, containerId) {
    ...
    // Распределяем темы по секциям
    data.forEach(theme => {
        // Определяем секцию на основе имени темы или других критериев
        let sectionName = 'Грамматика'; // По умолчанию
        if (theme.name.toLowerCase().includes('глагол')) {
            sectionName = 'Спряжение глаголов';
        } else if (theme.name.toLowerCase().includes('тем')) {
            sectionName = 'Практические темы';
        }
        sections[sectionName].push(theme);
    });

    // Создаем секции и добавляем темы
    Object.entries(sections).forEach(([sectionName, themes]) => {
        if (themes.length > 0) {
            ...
            title.className = 'section-title courses_titles';
